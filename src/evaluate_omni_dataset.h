...
///@INFO In truth we have only 4 robots in the dataset but since the robot IDs
/// go upto 5, we will keep a phantom robot: OMNI2
const std::size_t NUM_ROBOTS = 5;
///@INFO In truth we have 2 targets in the raw dataset but the 'pre-processed'
/// data does not contain the measurements of the blue ball from the robots (we
/// have that only for the orange ball). However, extend this to 2 in case you
/// do
/// your own pre-processing for the blue ball measurements using the raw images
/// from the robot's camera. Check raw dataset details for other related params
...
///@INFO Extrinsic stere calibration is performed on the fly (to avoid carrying
/// a lot of xml files and make this code more portable) and called at the
/// beginning of this node. Check method initializeCamParams()

// Left camera
double M1[] = { 952.364130338909490, 0, 638.552089454614702, 0,
                953.190083002521192, 478.055570057081638, 0, 0, 1 };
double D1[] = { -0.239034777360406, 0.099854524375872, -0.000493227284393,
                0.000055426659564, 0.000000000000000 };

// Right Camera
double M2[] = { 949.936867662258351, 0, 635.037661574667936, 0,
                951.155555785186380, 482.586148439616579, 0, 0, 1 };
double D2[] = { -0.243312251484708, 0.107447328223015, -0.000324393194744,
                -0.000372030056928, 0.000000000000000 };

// Some constants for drawing funcions... change these values if you want more
// points on the overlaid circle... an so on
static const int circlePointsPerPosition = 50;
static const int targetPntsPos = 1;
static const int arrowPointsPerPosition = 20;
static const int totPntsPerPos =
    1 + circlePointsPerPosition + arrowPointsPerPosition;
...
class ImageOverlayer
{
  ...
  Publisher errorPublisherOMNI1, errorPublisherOMNI3, errorPublisherOMNI4,
      errorPublisherOMNI5;
  Publisher orangeBallError;
  ros::Time timeBallNotFound;
  // Main image
  IplImage img;
...
public:
  ImageOverlayer(char* camImageFolderPath)
  {
    ...
    cvNamedWindow(
        "Overlaid Estimates on the Grount Truth Images of Right GT Camera");

    // Don't change the topic of this subscriber. It is the topic from GT
    // ROSbags.
    Image_sub = n.subscribe<read_omni_dataset::LRMGTData>(
        "gtData_4robotExp", 1000,
        boost::bind(&ImageOverlayer::gtDataCallback, this, _1, &img));
    ...
    omni_sub = n.subscribe<read_omni_dataset::RobotState>(
        "estimated_omni_poses", 1000,
        boost::bind(&ImageOverlayer::omniCallback, this, _1, &img));
    ...
    target1_sub = n.subscribe<read_omni_dataset::BallData>(
        "estimatedOrangeBallState", 1000,
        boost::bind(&ImageOverlayer::target1Callback, this, _1, &img));

    // On these topics the eindividual errors in estimation is published
    errorPublisherOMNI1 =
        n.advertise<std_msgs::Float32>("/omni1/EstimationError", 1000);
    errorPublisherOMNI3 =
        n.advertise<std_msgs::Float32>("/omni3/EstimationError", 1000);
    errorPublisherOMNI4 =
        n.advertise<std_msgs::Float32>("/omni4/EstimationError", 1000);
    errorPublisherOMNI5 =
        n.advertise<std_msgs::Float32>("/omni5/EstimationError", 1000);
    orangeBallError =
        n.advertise<std_msgs::Float32>("/OrangeBallEstimationError", 1000);
    ...
    foundOMNI_GT[0] = false;
    foundOMNI_GT[1] = false;
    foundOMNI_GT[2] = false;
    foundOMNI_GT[3] = false;
    foundOMNI_GT[4] = false;
  ...
  /// Important initializer: computes the reprojection matrices based on the
  /// stereo calibration. Do not change this method for the LRM GT images!!!
  /// Port it to other stereosystems if and when necessary
  void initializeCamParams(void);

  /// Overlay the colored circle, and arrow for the updated robot pose
  void OverlayEstimatedRobotPose(double, double, double, double, CvScalar,
                                 IplImage*);
  ...
  void OverlayEstimatedTargetPosition(double, double, double, CvScalar,
                                      IplImage* baseImage);

  /// Overlay the colored circle for the GT target pose
  void OverlayGTTargetPosition(double, double, double, CvScalar,
                               IplImage* baseImage);
};
